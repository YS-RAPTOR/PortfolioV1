...
export default class SkillCubeRender {
    ...
    async loadText(font: any, size: number) {
        ...
        this.skills.forEach((skill, index) => {
            ...
            const textMaterial = new THREE.MeshBasicMaterial({ color: 0xbef264, fog: true });
    ...
    getSphere = (radius: number, noOfPoints: number): { x: number, y: number, z: number }[] => {
        var points: { x: number, y: number, z: number }[] = []

        var epsilon = 0

        if (noOfPoints >= 600000)
            epsilon = 214
        else if (noOfPoints >= 400000)
            epsilon = 75
        else if (noOfPoints >= 11000)
            epsilon = 27
        else if (noOfPoints >= 890)
            epsilon = 10
        else if (noOfPoints >= 177)
            epsilon = 3.33
        else if (noOfPoints >= 24)
            epsilon = 1.33
        else
            epsilon = 0.33
        ...
        for (let i = 0; i < noOfPoints; i++) {
            ...
            const phi = Math.acos(1 - 2 * (i + epsilon) / (noOfPoints - 1 + 2 * epsilon))
            points.push({ x: Math.cos(theta) * Math.sin(phi) * radius, y: Math.sin(theta) * Math.sin(phi) * radius, z: Math.cos(phi) * radius })
        }
    ...
    render = () => {
        this.meshes.forEach((mesh) => {
            ...
            mesh.lookAt(new THREE.Vector3(this.temp.x, this.temp.y, this.camera.position.z));
        });

        this.scene.rotateOnWorldAxis(new THREE.Vector3(0, 1, 0), this.velocity.x);
