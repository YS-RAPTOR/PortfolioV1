...
const SkillCube = ({ skills, size }: { skills: string[], size: number }) => {
    ...
    useEffect(() => {
        const SkillSphereCanvas = document.getElementById("Skills") as HTMLCanvasElement;

        if (!SkillSphereCanvas) return;

        // Setting up canvas
        SkillSphereCanvas.width = AccountForPadding(Math.min(window.innerWidth, size));
        SkillSphereCanvas.height = AccountForPadding(Math.min(window.innerWidth, size));
