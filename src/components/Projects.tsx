...
const InitialDesc =
	"Click on a specific project on the menu to learn more about it.";

const Projects = ({
	projectsInfoResponse,
}: {
	projectsInfoResponse: CollectionEntry<"projects-data">[];
}) => {
	...
	var selectedProject = projectsInfo[selected];
	if (selectedProject === undefined) {
		selectedProject = {} as CollectionEntry<"projects-data">;
	}
								...
								{selected != -1 && (
									<div className="mt-auto flex flex-col gap-y-2 p-2 pt-4">
										<div className="bg-lime-300 flex flex-wrap flex-grow rounded h-fit p-1 justify-center items-center text-center gap-x-1">
											{selectedProject.data.technologies.map(
												(tech, index) => {
													return (
														<Tech
															key={index}
										...
										<div className="flex gap-x-5 items-center">
											<Social
												socialID={0}
												link={selectedProject.data.link}
											></Social>
											{selectedProject.data.demo && (
												<Social
													socialID={3}
													link={
														selectedProject.data
											...
											{/* {
                                                !selectedProject.data.draft && (
                                                    <a
                                                        href={`/PortfolioV1/Projects/${selectedProject.slug}`}
