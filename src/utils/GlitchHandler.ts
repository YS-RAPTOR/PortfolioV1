...
export default class GlitchHandler {
	...
	GlitchRandom = () => {
		...
		if (
			...
			this.i >= this.iterations * this.text.length * randomMultiplier
		) {
			this.setDisplayedText(this.text);
			clearInterval(this.interval);
		}
	...
	GetRandomChar = () => ALPHABET[Math.floor(Math.random() * ALPHABET.length)];
	GetRandomString = () =>
		Array.from(this.text)
			.map((char) =>
				char === this.space ? this.space : this.GetRandomChar()
			)
			.join("");
}
